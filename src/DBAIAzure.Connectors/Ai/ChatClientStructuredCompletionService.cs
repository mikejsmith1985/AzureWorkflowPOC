...
// the GA replacement for the SK-era forced-tool implementation on AnthropicChatCompletionService.
...
/// <summary>
/// Produces schema-bound structured output through <see cref="IChatClient"/> by asking the model for a
/// response constrained to a JSON schema (<see cref="ChatResponseFormat.ForJsonSchema(JsonElement, string, string)"/>)
/// and binding the returned JSON to the typed record. This preserves the Article VII "no free-text
/// parsing" guarantee while moving off the provider-specific SK forced-tool path — any provider whose
