...
- [ ] T001 Move aside any existing dev DB so the new `CostLedgerEntry` table provisions cleanly: rename
  `src/DBAIAzure.Web/pipeline.db*` to `*.bak` (`EnsureCreated`, not migrations).
- [ ] T002 Add `WebhookSecrets:Telemetry` to `appsettings`/config docs for the dev-usage endpoint.
...
- [ ] T008 Add the three custom fields (`Custom.CostBindingKey`, `Custom.AIRuntimeCostUSD`,
...
- [ ] T010 [P] [US1] Unit test: `ValidationStep` fails DoR when `state.CostBindingKey` is missing/invalid,
  in `tests/DBAIAzure.Tests/CostTracking/BindingDoREnforcementTests.cs`.
- [ ] T011 [P] [US1] Unit test: `SpecKitSignalMapper`/intake places a minted key on the initial state,
...
- [ ] T013 [US1] Add `CostBindingKey` to `src/DBAIAzure.Core/Models/PhaseHandlerState.cs`.
- [ ] T014 [US1] Mint at intake: `SpecKitWebhookController` injects `IBindingKeyMinter`, mints, and
  `SpecKitSignalMapper.ToInitialState` carries it onto the state.
- [ ] T015 [US1] Enforce at DoR: `src/DBAIAzure.Processes/Steps/ValidationStep.cs` fails when the binding
  key is absent/invalid.
- [ ] T016 [US1] Persist on creation (ADO): `CreateWorkItemStep` writes `Custom.CostBindingKey` via
...
- [ ] T018 [P] [US2] Unit test: one run → one runtime entry on the anchor; multi-item run not duplicated
  (FR-008), in `tests/DBAIAzure.Tests/CostTracking/RuntimeAttributionTests.cs`.
- [ ] T019 [P] [US2] Unit test: per-item `Custom.AIRuntimeCostUSD` equals the ledger total for the key,
  in `tests/DBAIAzure.Tests/CostTracking/CostProjectionTests.cs`.

### Implementation
- [ ] T020 [US2] Replace the per-child telemetry stamping in `CreateWorkItemStep.WriteTelemetryAsync`
  with a single runtime `ICostLedger.AppendAsync` on the **anchor** work item (binding key + run cost).
- [ ] T021 [US2] Implement the per-item cost projection (recompute `Custom.AIRuntimeCostUSD` from
  `GetTotalsAsync` after append) in `src/DBAIAzure.Web/Services/CostProjectionService.cs`; wire into the
  write path.
- [ ] T022 [US2] Register ledger + projection in `src/DBAIAzure.Web/Program.cs` and inject into the
...
### Tests (write first)
- [ ] T023 [P] [US3] Unit test: ingest with a valid key → attributed `Development` entry, cost re-priced
  via `ModelPricing` when absent, in `tests/DBAIAzure.Tests/CostTracking/DevUsageIngestTests.cs`.
- [ ] T024 [P] [US3] Unit test: ingest with an unknown key → entry with `IsUnattributed = true`,
  response `attributed:false` (FR-010), same file.
- [ ] T025 [P] [US3] Unit test: missing/blank `X-Telemetry-Secret` → 401, in
  `tests/DBAIAzure.Tests/CostTracking/DevUsageAuthTests.cs`.

### Implementation
- [ ] T026 [P] [US3] Create `DevUsageIngestPayload` DTO in
  `src/DBAIAzure.Web/Integrations/Telemetry/DevUsageIngestPayload.cs`.
- [ ] T027 [US3] Implement `TelemetryIngestController` (`POST /api/telemetry/dev-usage`, secret-gated via
...
- [ ] T028 [US3] Extend the cost projection to recompute `Custom.AIDevCostUSD` from the ledger after a
...
- [ ] T029 [US4] Author the ADO Analytics (OData) saved query / Power BI report definition that sums
...
- [ ] T030 [P] Write the org-rollout runbook: how to configure a coding agent (Claude Code) to emit usage
  and bind it to a ticket key, in `docs/dev-agent-telemetry-setup.md`.
- [ ] T031 [P] Update `CHANGELOG.md` under `[Unreleased]`.
- [ ] T032 Run the full unit suite (`dotnet test tests/DBAIAzure.Tests`) — confirm green except the known
  pre-existing `ConnectorSettings_WhenSaveClicked` bUnit failure.
- [ ] T033 Code-quality pass against the constitution across all changed files.
...
- [ ] T036 [US1] **(C1)** Populate the map in `CreateWorkItemStep` (`PutAsync(bindingKey, anchorWorkItemId)`)
  when the work item is created, in `src/DBAIAzure.Processes/Steps/CreateWorkItemStep.cs`.
- [ ] T037 [US1] **(C2)** ServiceNow write-back of the binding key: first verify the ServiceNow connector
...
- [ ] T038 [P] [US2] **(U1)** Non-blocking test (FR-011): a throwing `ICostLedger`/projection does NOT
