...
- [X] T050 **Atomic cutover — SK removed.** Deleted all SK Steps, the `IntakePipelineBuilder`/`PhaseHandlerPipelineBuilder`/`WorkflowRuntimeBuilder`, the HITL/approval external channels, the two SK cost filters, `AnthropicChatCompletionService`/`HotReloadAnthropicService`, and `Events`/`WorkflowNodeEvents`/`WorkflowInputTranslator`; converted `FunctionTransformStep`/`FunctionDataStep` to pure static helpers. Removed every `Microsoft.SemanticKernel*` package reference and every `SKEXP0080` pragma/NoWarn across `src/` + `tests/`. All 3 orchestrators are MAF-only (no flag, no kernel factory); the design-time AI services (builder assistant, code generator, availability monitor, node realization) run on `IChatClient`. Grep gate `SemanticKernelRemovedGateTests` proves zero `Microsoft.SemanticKernel`/`SKEXP` matches across the tree (FR-003 / SC-002).
- [X] T051 Rewrote `src/DBAIAzure.Runner/Program.cs` to build the intake pipeline via `MafIntakeWorkflowFactory` and drive the `RequestPort` console HITL loop over a `MafWorkflowSession`, with the model client from `AnthropicChatClientProvider` (`IChatClient`).
