...
- [ ] T019 [US1] Port `IntakePipelineBuilder` → `WorkflowBuilder` graph (edges + labelled **conditional edges** — `AddEdge(src, tgt, condition, label)`; **no `AddSwitch` in GA 1.13.0**, see research D1-reality) in `src/DBAIAzure.Processes/Pipeline/Maf/MafIntakeWorkflowFactory.cs` (stub in place; T014 defines target).
