...
- [~] T032 [US2] Make checkpointing **live**: `MafWorkflowSession` threads a `CheckpointManager` (runs are checkpointed at every super-step) and exposes `ResumeAsync(workflow, checkpoint, manager)` for restart recovery; both orchestrators accept + pass the manager; `Program.cs` registers `EfCheckpointStore` + `CheckpointManager.CreateJson`. Proven: a paused orchestrated MAF run persists checkpoints for its session (`OrchestratorCheckpointingTests`). **Pending:** the startup hosted-service that iterates paused runs and calls `ResumeAsync` on boot (finishing touch; the visual `WorkflowRunRehydrationService` waits on the visual-orchestrator migration).
