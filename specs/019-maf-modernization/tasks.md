...
- [~] T017 [US1] Convert the stateless steps → MAF `Executor` in `src/DBAIAzure.Processes/Executors/` per `contracts/workflow-executor-mapping.md`. **Intake set DONE** (Intake/Validation/Estimation/Action/GapAnalysis + shared `ExecutorLlm`), each porting its SK step's exact prompt/parse; routing via `[SendsMessage]`/`[YieldsOutput]` + broadcast + conditional edges. **Pending:** ReadArtifacts, PhaseValidation, CreateWorkItem (phase-handler set — T015).
...
- [ ] T021 [US1] Port `WorkflowRuntimeBuilder` → build a `Workflow` at runtime from `WorkflowDefinition` (node→executor, edge→`AddEdge`, route port→**labelled conditional edge** `AddEdge(src, tgt, m => m.Port == label, label)`, `PortLabelsByNodeId` retained) in `src/DBAIAzure.Processes/Pipeline/Maf/MafWorkflowRuntimeFactory.cs` (stub in place; T016 defines target).
